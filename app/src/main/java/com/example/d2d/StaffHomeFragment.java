 ...
 public class StaffHomeFragment extends Fragment implements StaffQueueAdapter.OnStatusChangeListener {
     ...
     private void loadActiveOrdersFromDatabase() {
         ...
         // Seed mock orders once if the database is empty on first run
         android.content.SharedPreferences prefs = requireContext().getSharedPreferences("D2D_PREFS", android.content.Context.MODE_PRIVATE);
         boolean seeded = prefs.getBoolean("seeded_mock_orders", false);
         if (!seeded) {
             android.database.Cursor testCursor = dbHelper.getAllOrders();
              if (testCursor == null || testCursor.getCount() == 0) {
                  dbHelper.saveOrder("10001", "Casa Nova", "preparing", "R 0.00", "Jane Smith", "10001");
                  dbHelper.saveOrder("10002", "Organic Resto", "ready", "R 0.00", "John Doe", "10000");
                  dbHelper.saveOrder("10003", "Burger Joint", "preparing", "R 0.00", "Test User", "10002");
              }
             if (testCursor != null) {
                 testCursor.close();
             }
             prefs.edit().putBoolean("seeded_mock_orders", true).apply();
         }
